...
const ServiceRequest = () => {
  ...
  const [isUpdateServiceRequest, setIsUpdateServiceRequest] = useState(false);
  ...
  const handleResetColumns = (isFromToggleColumns) => {
    ...
    $('#data-table thead tr:eq(1) th').each(function (i) {
      ...
      if (title !== 'Actions') {
        if (title === 'CreatedAt') {
          $(this).html(
            '<button id="from_created_date">Select Range</button><div id="created_date_popup" style="display:flex" ><div ><span class="common-table__range"><label class="common-table__label">From Date:</label><input readOnly type="text" id="created_min" name="min" /></span><span class="common-table__range"><label class="common-table__label">To Date:</label><input readOnly type="text" id="created_max" name="max" /></span></div><i id="clear_created_date" class="fas fa-times overlay__cross-icon common-table__cross-btn" ></i></div>'
          );
        } else {
          $(this).html(
        ...
        document.getElementById('created_date_popup') ? document.getElementById('created_date_popup').style.display = 'none' : null;
      }
      else {
        $(this).html(
          <th ></th>
  ...
  const handleUpdateServiceRequest = () => {
    ...
    //   status: obj.selectedDropdown1Value.value
    // };
    // updateServiceRequest((response) => {
    //   const { success, message } = response;
      if (2===2) {
        // getData([], 0);
        setIsUpdateServiceRequest(false);
        setServiceRequestDetails({});
        setAlertText('Successefully Changed the Status !');
      } else {
        setAlertText('');
      }
  ...
  return (
    <div className='service-request'>
      <Name tabTitle='Customer' title='Service Request' />
      ...
      {isUpdateServiceRequest && (
        <PopUp
          onConfirm={handleUpdateServiceRequest} onCancel={() => { setIsUpdateServiceRequest(false); }} commonTitle='Update Service Request'
          isDropdown1Required={true} dropdown1Placeholder='Change Status' dropdown1List={dropdown1List} btnText='UPDATE' />
      )}
      {isShowLoader && <Loader />}
      {isImageViewerOpen && (
        <ImageViewer
